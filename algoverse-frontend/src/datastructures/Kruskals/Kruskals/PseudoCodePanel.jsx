...
const kruskalsPseudoCode = [
  { lineNumber: 1, code: "void kruskalMST(Graph g) {", action: "function_start" },
  { lineNumber: 2, code: "  List<Edge> edges = g.getAllEdges();", action: "init_edges" },
  { lineNumber: 3, code: "  sortEdgesByWeight(edges);", action: "sort_edges" },
  { lineNumber: 4, code: "  UnionFind uf = new UnionFind(g.nodes);", action: "init_union_find" },
  { lineNumber: 5, code: "  for (Edge e : edges) {", action: "for_each_edge" },
  { lineNumber: 6, code: "    if (!uf.connected(e.u, e.v)) {", action: "check_cycle" },
  { lineNumber: 7, code: "      MST.add(e);", action: "add_to_mst" },
  { lineNumber: 8, code: "      uf.union(e.u, e.v);", action: "union_nodes" },
  { lineNumber: 9, code: "      if (MST.size == g.nodes - 1) break;", action: "check_complete" },
  { lineNumber: 10, code: "    }", action: "end_if" },
  { lineNumber: 11, code: "  }", action: "end_for" },
  ...
  return (
    <div className={`bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl border border-gray-700 shadow-lg ${className}`}>
      ...
      <div className="p-2 max-h-[600px] overflow-auto">
        {kruskalsPseudoCode.map((step, index) => (
