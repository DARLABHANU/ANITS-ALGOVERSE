...
const bubbleSortCode = [
  { lineNumber: 1, code: "void bubbleSort(int arr[], int n) {", action: "function_start" },
  { lineNumber: 2, code: "  for (int i = 0; i < n - 1; i++) {", action: "outer_loop_start" },
  { lineNumber: 3, code: "    for (int j = 0; j < n - i - 1; j++) {", action: "inner_loop_start" },
  { lineNumber: 4, code: "      if (arr[j] > arr[j + 1]) {", action: "compare" },
  { lineNumber: 5, code: "        swap(arr[j], arr[j + 1]);", action: "swap" },
  { lineNumber: 6, code: "      }", action: "end_if" },
  { lineNumber: 7, code: "    }", action: "end_inner_loop" },
  { lineNumber: 8, code: "  }", action: "end_outer_loop" },
  { lineNumber: 9, code: "}", action: "function_end" }
];

const BubbleSortPseudoCodePanel = ({ className }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (typeof getCurrentStepData === 'function') {
        ...
        // ✅ Only update currentStepData if line changed
        ...
        // 🕒 Remove variable highlight after delay
  ...
  const highlightedLineIndex = (currentStepData?.codeLineIndex ?? -1) + 1;
...
export default BubbleSortPseudoCodePanel;
